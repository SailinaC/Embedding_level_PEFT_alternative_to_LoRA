...
OUTPUT_DIR = Path("outputs/approach7_all_embeddings_except_cls_sep")
...
def save_checkpoint(step, model, optimizer, scheduler, scaler, initial_embedding):
    ...
    checkpoint = {
        ...
        "config": {
            ...
            "excluded_token_ids": [CLS_ID, SEP_ID],
            "trainable_embedding_rows": embedding_weight.shape[0] - 2,
            "trainable_parameters": (embedding_weight.shape[0] - 2) * embedding_weight.shape[1]
        }
    }
...
trainable_params = (embedding_weight.shape[0] - 2) * embedding_weight.shape[1]
total_params = sum(p.numel() for p in model.parameters())

print("Excluded token ids:", [CLS_ID, SEP_ID], flush=True)
print("Excluded tokens:", tokenizer.convert_ids_to_tokens([CLS_ID, SEP_ID]), flush=True)
print("Effective trainable embedding parameters:", trainable_params, flush=True)
print("Total model parameters:", total_params, flush=True)
print("Effective trainable percentage:", 100.0 * trainable_params / total_params, flush=True)
...
print("CLS change:", embedding_diffs[CLS_ID].item(), flush=True)
print("SEP change:", embedding_diffs[SEP_ID].item(), flush=True)

changed_rows = torch.nonzero(embedding_diffs > 1e-8).squeeze(-1).cpu().tolist()

print("Changed embedding rows:", len(changed_rows), flush=True)

...
MAX_SEQ_LENGTH = 128
LEARNING_RATE = 1e-4
OUTPUT_DIR = "outputs/approach3_top3_attention"
...
for name, param in model.named_parameters():
    if name in TARGET_MATRICES:
        param.requires_grad = True
        trainable_params_count += param.numel()
        found_matrices.append(name)

missing_matrices = sorted(set(TARGET_MATRICES) - set(found_matrices))

print("Target matrices:", TARGET_MATRICES)
print("Found matrices:", found_matrices)
print("Missing matrices:", missing_matrices)
print("Total trainable parameters:", trainable_params_count)

if missing_matrices:
    raise RuntimeError(f"Missing target matrices: {missing_matrices}")
...
for name, param in model.named_parameters():
    if name in initial_trainable_state:
        diff = torch.norm(param.detach() - initial_trainable_state[name]).item()
        print(f"Matrix change for {name}: {diff:.8f}")

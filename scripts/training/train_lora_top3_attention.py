...
OUTPUT_DIR = Path("outputs/approach6_lora_top3_attention")
...
def apply_lora_top3(model):
    ...
    for module_name in TARGET_MODULES:
        ...
        set_module_by_name(
            model,
            module_name,
            LoRALinear(base_layer, LORA_R, LORA_ALPHA, LORA_DROPOUT)
        )
...
target_names = apply_lora_top3(model)
model.to(device)

trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
total_params = sum(p.numel() for p in model.parameters())

print("LoRA target matrices:", target_names, flush=True)
...
print("Finished LoRA top-3 attention training", flush=True)

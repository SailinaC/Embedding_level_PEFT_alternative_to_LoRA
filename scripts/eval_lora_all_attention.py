...
CHECKPOINT_PATH = "outputs/approach5_lora_all_attention/final/trainable_checkpoint.pt"
...
RESULTS_JSONL = OUTPUT_DIR / "lora_all_attention_results.jsonl"
RESULTS_CSV = OUTPUT_DIR / "lora_all_attention_results.csv"
...
def apply_lora_all_attention(model, r, alpha, dropout):
    for layer in model.bert.encoder.layer:
        layer.attention.self.query = LoRALinear(layer.attention.self.query, r, alpha, dropout)
        layer.attention.self.key = LoRALinear(layer.attention.self.key, r, alpha, dropout)
        layer.attention.self.value = LoRALinear(layer.attention.self.value, r, alpha, dropout)
        layer.attention.output.dense = LoRALinear(layer.attention.output.dense, r, alpha, dropout)

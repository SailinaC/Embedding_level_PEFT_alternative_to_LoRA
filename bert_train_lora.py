...
# Setup arguments (keeping same as prompt script for consistency)
...
parser.add_argument('--learning_rate', type=float, default=3e-4) # LoRA usually uses higher LR than full fine-tuning
...
# Data loading function (identical to prompt script)
...
peft_config = LoraConfig(
    ...
    lora_dropout=0.1,
    target_modules=["query", "value"]  # Standard for BERT LoRA
)
...
# Training loop (identical to prompt script)
...
for k in range(total_passes):
    ...
    if (k + 1) % args.accumulation_steps == 0:
        ...
        if opt_step % 10000 == 0:
            model.save_pretrained(f'data/checkpoint-lora-{opt_step}')

        if opt_step >= args.num_train_steps:
            break

# Save final model
model.save_pretrained(f'data/bert-final-lora-{args.name}')
